...
contract NavIssuanceModuleUSDC is Script, ContractAddresses {
    ...
    function run() external {
        ...
        mintTokenSet(deployerPrivateKey, usdcBtcLink, mockUSDC, usdcValue, deployer);
    ...
    function mintTokenSet(
        ...
        uint256 minAmount = getMinSetTokenReceiveQuantity(setToken, reserveAsset, reserveAssetQuantity);
        vm.startBroadcast(privateKey);
        customOracleNavIssuanceModule.issue(setToken, reserveAsset, reserveAssetQuantity, minAmount, to);
        vm.stopBroadcast();
        console.log();
    }

    function getMinSetTokenReceiveQuantity(ISetToken setToken, address reserveAsset, uint256 reserveAssetQuantity)
        public
        view
        returns (uint256 minAmount)
    {
        ...
        minAmount =
            customOracleNavIssuanceModule.getExpectedSetTokenIssueQuantity(setToken, reserveAsset, reserveAssetQuantity);
